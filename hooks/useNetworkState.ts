...
export function useNetworkState(onNetworkRestore?: () => void) {
  ...
  // Truco 1: Guardar la función en una referencia para no meterla en el useEffect
  ...
  useEffect(() => {
    ...
    try {
      ...
      const unsubscribe = NetInfo.addEventListener((state: any) => {
        ...
        // Truco 2: Solo actualizar si los valores REALMENTE cambiaron (evita re-renders infinitos)
        ...
        // Truco 3: Usar el estado previo (prev) para no depender de variables externas
  ...
  }, []); // <-- ¡CORCHETES VACÍOS! Esto detiene el bucle infinito de raíz.
